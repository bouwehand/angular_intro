...
class PhpTumblr_Model_ReadTumblr {
	...
	public function __construct($sTumblrID = null, $sHTTPUserAgent = 'phpTumblr') {
		if ($sTumblrID == null) {
            unset($this);
            throw new Zend_Exception('no tumblr id given');
        }
		...
		$oNetHttp = new PhpTumblr_Model_NetHttp($sTumblrURL);
		$oNetHttp->setUserAgent($sHTTPUserAgent);
        $oNetHttp->useGzip(true);
		$oNetHttp->get('/');
    ...
    public function getPosts($nStart = 0,$mNum = 20,$sType = null,$nID = null,$sTagged = null, $sSearch = null) {
		...
		if ($nStart != null && is_numeric($nStart)) {
            $aParams['start'] = $nStart;
        } else {
            $aParams['start'] = 0;
        }
		if (!is_numeric($mNum)) {
                    ...
                    case 'photo' :
                        ...
                        if ($post['photos'] != array()) {
                            ...
                            foreach ($post['photos'] as $picId => $photo) {
                                $aPosts[$pid]['content']['photos'][$picId] = array(
                                    'photo-caption'     => $photo['caption'],
                                    'photo-width'       => $photo['width'],
                                    'photo-height'      => $photo['height'],
                                    'url-1280'          => $photo['photo-url-1280'],
                                    'url-500'           => $photo['photo-url-500'],
                                    'url-400'           => $photo['photo-url-400'],
                                    'url-250'           => $photo['photo-url-250'],
                                    'url-100'           => $photo['photo-url-100'],
                                    'url-75sq'          => $photo['photo-url-75']
                                );
                            }
                        } else {
